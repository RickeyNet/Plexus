...
- [ ] **IP address management (IPAM) integration** — add lightweight IPAM with subnet tracking, utilization visualization, and conflict detection; or integrate with external IPAM (Infoblox, NetBox, phpIPAM) via API to pull/push address assignments; display IP utilization in inventory and topology views. *Why: IP conflicts and exhaustion cause hard-to-diagnose connectivity issues. Having address management visibility alongside network monitoring eliminates the need to cross-reference a separate IPAM tool during troubleshooting.*
  ...
  - [ ] Next slice (phase F): DHCP scope and lease integration — pull DHCP scope utilization and active leases from ISC DHCP, Windows DHCP, and Infoblox; correlate leases against discovered device inventory; alert on scope exhaustion and unauthorized leases.
  - [ ] Next slice (phase G): VLAN/VRF-aware subnet scoping — tag subnets with VLAN ID and VRF name so overlapping RFC1918 ranges across different VRFs do not false-positive as conflicts; display VRF context in IPAM views and device detail panel.
  - [ ] Next slice (phase H): IPAM-driven provisioning — reserve next-available IP from external IPAM when onboarding a new device or interface; support reservation rollback on provisioning failure; expose `/api/ipam/allocate` endpoint for automation/playbook use.
  - [ ] Next slice (phase I): historical IP allocation tracking — store time-series of subnet utilization and per-IP assignment history; support "who had this IP on date X" lookups for security/forensics use cases.
  - [ ] Next slice (phase J): IPAM reporting and exports — subnet utilization reports, free-space forecasting, and exhaustion projections with CSV/PDF export; scheduled delivery to capacity-planning stakeholders.
