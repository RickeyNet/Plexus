...
- [x] Add NetFlow/sFlow/IPFIX collection and traffic analysis.
  ...
  - [ ] Phase B — exporter→host resolution: add in-memory `_exporter_cache: dict[str, int]` populated from `hosts.ip_address` at startup and refreshed on host CRUD; fill `host_id` in `datagram_received` instead of `None` so per-device queries actually work; add `flow_exporters` table (`exporter_ip`, `host_id`, `flow_type`, `packets_received`, `last_record_at`, `sampling_rate`, `first_seen`, `last_seen`) and update on every packet so the UI can show who's exporting without scanning `flow_records`.
