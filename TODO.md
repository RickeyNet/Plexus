...
- [x] Add NetFlow/sFlow/IPFIX collection and traffic analysis.
  ...
  - [ ] Phase C — sFlow v5 parser: implement `parse_sflow(data, addr)` for header + flow_sample (sampled raw Ethernet/IP) + counter_sample; open second UDP socket on 6343 in `start_flow_collector`; normalize sampled records into the same `flow_records` shape used by NetFlow.
