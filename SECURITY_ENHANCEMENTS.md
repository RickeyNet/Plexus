...
## Credential isolation audit (security review pass 3)

CRUD endpoints on `/api/credentials/*` correctly scope to `owner_id`, but every *operational* endpoint that consumes a credential loads it via `db.get_credential_raw(credential_id)` with no ownership check. Any authenticated user can execute jobs, deployments, backups, compliance scans, upgrades, etc. using another user's (or admin's) credentials by submitting their `credential_id`. This is a privilege-escalation IDOR across multiple feature areas.

### Critical

All nine items below were re-verified 2026-07-06: every operational endpoint
now routes through `require_credential_access()` (see High). The one gap
found in the sweep - compliance remediation - was fixed the same day.
Regression tests: `tests/test_credential_isolation.py`.

- [x] **Job launch - default-credential bypass** - Fixed: `netcontrol/routes/jobs.py:823-826` routes both the explicit `body.credential_id` path and the `default_credential_id` fallback through `require_credential_access(cred_id, session=session)`.
- [x] **Job queue processor - no ownership check at runtime** - Fixed: `netcontrol/routes/jobs.py:378-394` validates `credential_id` (or the default) with `submitter_username=next_job["launched_by"]` before decrypting; on failure the job is marked `failed` with an explanatory job event.
- [x] **Deployments - no ownership check on create/execute/rollback** - Fixed: create validates at `deployments.py:697`; execute (`:857`) and rollback (`:1004`) go through `_resolve_deployment_credential` which validates against the deployment's `created_by`.
- [x] **Config backup policies - no ownership check on create/update/run** - Fixed: policy create (`config_backups.py:73`), update (`:111`), run-now (`:378`), restore (`:437`), and the scheduler loop (`:523`, `submitter_username=policy["created_by"]`, fails closed).
- [x] **Config drift snapshot capture - no ownership check** - Fixed: all capture/revert paths validate against the session (`config_drift.py:690,717,753,783,1037,1169`). No submitter column needed: the scheduled drift loop only re-analyzes existing snapshots and never opens an SSH session, so it consumes no credential.
- [x] **Compliance assignments/scans/remediation - no ownership check** - Fixed: assignment create (`compliance.py:440`), update (`:474`), scan-now (`:580`), single/bulk scans (`:648,:682`), scheduled scans (`:250`, against `assigned_by`). Remediation (`:822`) was still unvalidated when re-audited and was fixed 2026-07-06 with a wiring regression test.
- [x] **Upgrades - no ownership check** - Fixed: campaign create (`upgrades.py:847`), update (`:931`), execute (`:1099`) and scheduled-activate rehydration (`:310`) via `_resolve_campaign_credential`, which validates against the campaign's `created_by`; device-level transfer/activate receive the already-validated credential object.
- [x] **Risk analysis - no ownership check** - Fixed: `risk_analysis.py:370` (the only credential-bearing endpoint).
- [x] **Inventory operations - no ownership check** - Fixed: single serial fetch (`inventory.py:1065`) and bulk group fetch (`:1105`). Discovery endpoints take no `credential_id` (SNMP-profile based), so nothing to validate there.

### High

- [x] **Centralize credential ownership validation** - Added `require_credential_access(credential_id, *, session=None, submitter_username=None, allow_service=False)` in `netcontrol/routes/shared.py`. Final policy (tightened from the original draft): user credentials are strictly per-owner - admin role does NOT grant use of another user's credential; only API-token callers bypass; unowned credentials (NULL `owner_id`) are API-token-only; service credentials (`is_service=1`) require the `allow_service` opt-in AND admin context. Helper accepts either a live session dict (HTTP paths) or a submitter username (queue/scheduler paths) and raises HTTPException on any failure. All operational endpoints now use it (verified 2026-07-06).
- [x] **Default-credential fallback policy decision** - Chose option (a): the default credential is only usable by callers who actually own it (or are admin). `netcontrol/routes/jobs.py:586-596` now routes both the explicit and default paths through `require_credential_access`, closing the bypass while keeping the convenience for users who configured a personal default or for admins. No schema or UX change required.
- [x] **Queue/scheduler owner model** - Surveyed all detached-execution tables: `jobs.launched_by` (always populated, defaults to 'admin'), `deployments.created_by`, `config_backup_policies.created_by`, `compliance_profile_assignments.assigned_by`, and `upgrade_campaigns.created_by` all store the submitter username as TEXT; `config_drift_*` has no submitter column for automated captures. The job queue processor (`jobs.py:241-261`) now re-validates credential ownership against `next_job["launched_by"]` and fails the job with a clear audit event if the check fails. All detached-execution points now apply the same `submitter_username=` pattern. `config_drift` turned out not to need a submitter column: its scheduled loop only re-analyzes stored snapshots and never opens a credentialed session (verified 2026-07-06).

### Medium

- [x] **Monitoring uses system-default credential** - Decided and implemented: monitoring (and the MAC poller) prefer a dedicated `service_credential_id` (a credential flagged `is_service=1`, reserved for Plexus-internal work) and fall back to the legacy `default_credential_id` only for un-migrated deployments (`monitoring.py:809-816,1040-1046`). Service credentials are rejected for user-facing flows unless the call site opts in with `allow_service=True`, and even then require admin context.
- [x] **API-token = admin on credentials** - Documented 2026-07-06: `AUTHORIZATION_MATRIX.md` now has an "API-token callers" section stating the token is admin-equivalent by design (role and ownership checks bypass), why (CI/CD + federation), the handling consequences, and that per-token scoping would need a token-scopes design first.
- [x] **Integration tests for credential isolation** - Added `tests/test_credential_isolation.py` (2026-07-06): full policy matrix for `require_credential_access` (owner/non-owner/admin/unowned/API-token), service-credential opt-in rules, submitter-path validation incl. deleted-submitter and mismatch fail-closed (the queue-processor semantics), audit-event assertions, and a wiring regression for compliance remediation. Full per-endpoint HTTP-level coverage remains tracked in `TODO.md` Batch 5.
- [x] **Audit logging for credential use** - Implemented 2026-07-06 inside `require_credential_access` so every consumer gets it for free: each allow emits a `credential`/`use` audit event (caller, credential_id, owner_id, service flag) and each deny emits `credential`/`use_denied` with the reason. Grants that don't rest on plain ownership carry an explicit `override=` marker (`api-token`, `admin-service-cred`) so legitimate bypasses are distinguishable from regressions.

## Full-codebase security review (pass 4)

### Critical

- [x] **Dev-bootstrap auth enabled by default in production containers** - Fixed (verified 2026-07-06): `templates/run.py` no longer calls `os.environ.setdefault(...)` for `APP_ENV` / `PLEXUS_DEV_BOOTSTRAP` / `PLEXUS_BREAKGLASS_LOCAL_ADMIN` - dev bootstrap now requires explicit opt-in (`APP_ENV=dev|development|local` or `PLEXUS_DEV_BOOTSTRAP=true`, checked in `auth.py:_dev_bootstrap_enabled`), so containers boot with the well-known `admin/netcontrol` login disabled and the run.py banner explains this. The deterministic password reset inside `_authenticate_dev_bootstrap` still exists but is only reachable in explicitly-enabled dev mode (it is the feature: a deterministic local dev login). Residual notes: `PLEXUS_BREAKGLASS_LOCAL_ADMIN` defaults to true, but breakglass verifies the *real* local admin password hash - it bypasses "local fallback disabled" during LDAP/RADIUS outages, not authentication itself; first-boot admin password is random unless `PLEXUS_INITIAL_ADMIN_PASSWORD` is set (see pass-1 Critical), with `scripts/reset_admin_password.py` as the recovery path. The Dockerfile keeping `templates/run.py` as CMD is now safe since the script no longer mutates auth env vars.
