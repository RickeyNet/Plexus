...
BUILTIN_PROFILES = [
    ...
    (
        ...
        [
            ...
            },
            {
                ...
                "type": "regex_match",
                "pattern": r"ip ssh time-out\s+\d+",
                "remediation": ["ip ssh time-out 60"],
            },
            {
                "name": "SSH auth retries limited",
                "type": "regex_match",
                "pattern": r"ip ssh authentication-retries\s+[1-3]$",
            ...
            {
                "name": "VTY requires SSH",
                "type": "regex_match",
                "pattern": r"line vty[\s\S]*?transport input ssh",
                "remediation": ["line vty 0 4", "transport input ssh", "line vty 5 15", "transport input ssh"],
            },
            {
                "name": "Console timeout set",
                "type": "regex_match",
                "pattern": r"line con 0[\s\S]*?exec-timeout\s+\d+",
                "remediation": ["line con 0", "exec-timeout 10 0"],
            },
            {
                "name": "VTY timeout set",
                "type": "regex_match",
                "pattern": r"line vty[\s\S]*?exec-timeout\s+\d+",
                "remediation": ["line vty 0 4", "exec-timeout 10 0", "line vty 5 15", "exec-timeout 10 0"],
            },
            {
                "name": "No IP finger service",
                "type": "must_not_contain",
                "pattern": "ip finger",
                "remediation": ["no ip finger"],
            },
            {
                "name": "No IP identd service",
                "type": "must_not_contain",
                "pattern": "ip identd",
                "remediation": ["no ip identd"],
            },
            {
                "name": "No TCP small servers",
                "type": "must_not_contain",
                "pattern": "service tcp-small-servers",
                "remediation": ["no service tcp-small-servers"],
            },
            {
                "name": "No UDP small servers",
                "type": "must_not_contain",
                "pattern": "service udp-small-servers",
            ...
            {
                "name": "Timestamps on log messages",
                "type": "must_contain",
            ...
            {
                "name": "HTTPS server disabled (or secured)",
                "type": "regex_match",
                "pattern": r"no ip http secure-server|ip http secure-server",
    ...
    (
        ...
        [
            ...
            {
                "name": "Logging buffered configured",
                "type": "regex_match",
                "pattern": r"logging buffered\s+\d+",
                "remediation": ["logging buffered 64000 informational"],
            },
            {
                "name": "Logging to syslog server",
                "type": "regex_match",
                "pattern": r"logging\s+(host\s+)?\d+\.\d+\.\d+\.\d+",
                "remediation": ["logging host 10.0.0.50"],
            },
            {
                "name": "Logging console level",
                "type": "regex_match",
                "pattern": r"logging console\s+(critical|alerts|emergencies)",
            ...
            {
                "name": "SNMPv3 configured",
                "type": "regex_match",
                "pattern": r"snmp-server group\s+\S+\s+v3\s+(auth|priv)",
                "remediation": ["snmp-server group PLEXUS-RO v3 priv"],
            },
            {
                "name": "CDP disabled globally (or per-interface)",
                "type": "regex_match",
                "pattern": r"no cdp run|no cdp enable",
                "remediation": ["no cdp run"],
            },
            {
                "name": "LLDP disabled globally (or per-interface)",
                "type": "regex_match",
                "pattern": r"no lldp run|no lldp transmit",
                "remediation": ["no lldp run"],
            },
            {
                "name": "Login banner present",
                "type": "regex_match",
                "pattern": r"banner (login|motd)\s+",
                "remediation": [
                    "banner login #",
    ...
    (
        ...
        "CIS Cisco IOS Benchmark v4.1 - data plane security. Covers uRPF, ACLs, CEF, and anti-spoofing protections.",
        "high",
        [
            {"name": "CEF enabled", "type": "regex_match", "pattern": r"ip cef|ipv6 cef", "remediation": ["ip cef"]},
            {
                "name": "No IP directed broadcast",
                "type": "must_contain",
                "pattern": "no ip directed-broadcast",
                "remediation": ["no ip directed-broadcast"],
            },
            {
                "name": "No IP proxy ARP (global or interface)",
                "type": "regex_match",
                "pattern": r"no ip proxy-arp",
                "remediation": ["no ip proxy-arp"],
            },
            {
                "name": "uRPF configured on untrusted interfaces",
                "type": "regex_match",
    ...
    (
        ...
        [
            {
                "name": "STP portfast on access ports",
                "type": "regex_match",
                "pattern": r"spanning-tree portfast\s*(edge)?",
                "remediation": ["spanning-tree portfast default"],
            },
            {
                "name": "BPDU guard enabled",
                "type": "regex_match",
                "pattern": r"spanning-tree\s+(portfast\s+)?bpduguard\s+(enable|default)",
                "remediation": ["spanning-tree portfast bpduguard default"],
            },
            {
                "name": "STP root guard or loop guard",
                "type": "regex_match",
                "pattern": r"spanning-tree guard (root|loop)",
            ...
            },
            {
                ...
                "type": "regex_match",
                "pattern": r"ip arp inspection vlan\s+\S+",
                "remediation": ["ip arp inspection vlan 1-4094"],
            },
            {
                "name": "Storm control configured",
                "type": "regex_match",
                "pattern": r"storm-control broadcast level\s+\d+",
                "remediation": None,
            },
            {
            ...
            },
            {
                ...
                "type": "regex_match",
                "pattern": r"switchport trunk native vlan\s+(?!1\b)\d+",
                "remediation": None,
            },
            {
                "name": "VTP mode transparent or off",
                "type": "regex_match",
                "pattern": r"vtp mode\s+(transparent|off)",
                "remediation": ["vtp mode transparent"],
            },
            {
                "name": "Port security enabled on access ports",
                "type": "regex_match",
                "pattern": r"switchport port-security",
                "remediation": None,
            },
        ],
    ),
    ...
    (
        ...
        [
            ...
            {
                "name": "AAA authentication login defined",
                "type": "regex_match",
                "pattern": r"aaa authentication login\s+\S+",
                "remediation": ["aaa authentication login default local"],
            },
            {
                "name": "AAA authorization exec",
                "type": "regex_match",
                "pattern": r"aaa authorization exec\s+",
                "remediation": ["aaa authorization exec default local"],
            },
            {
                "name": "AAA accounting commands",
                "type": "regex_match",
                "pattern": r"aaa accounting commands\s+",
                "remediation": ["aaa accounting commands 15 default start-stop group tacacs+"],
            },
            {
                "name": "TACACS or RADIUS server configured",
                "type": "regex_match",
                "pattern": r"(tacacs\s+server\s+\S+|tacacs-server\s+host\s+\S+|radius\s+server\s+\S+|radius-server\s+host\s+\S+)",
                "remediation": None,
            },
            {
                "name": "Local fallback user exists",
                "type": "regex_match",
                "pattern": r"username\s+\S+\s+privilege\s+15\s+secret",
                "remediation": None,
            },
            {
        ...
        [
            {
                ...
                "type": "regex_match",
                "pattern": r"ip ospf (message-digest-key|authentication)",
                "remediation": None,
            },
            {
                "name": "EIGRP authentication configured",
                "type": "regex_match",
                "pattern": r"ip authentication mode eigrp|ip authentication key-chain eigrp",
                "remediation": None,
            },
            {
                "name": "BGP neighbor password set",
                "type": "regex_match",
                "pattern": r"neighbor\s+\S+\s+password",
                "remediation": None,
            },
            {
                "name": "No default OSPF passive interface",
                "type": "regex_match",
                "pattern": r"passive-interface (default|Vlan|Loopback)",
                "remediation": ["router ospf 1", "passive-interface default"],
            },
            {
                "name": "Prefix lists or route maps on BGP",
                "type": "regex_match",
                "pattern": r"neighbor\s+\S+\s+(prefix-list|route-map)",
                "remediation": None,
            },
        ],
    ),
    ...
    (
        ...
        [
            {
                "name": "No DES encryption policies",
                "type": "must_not_contain",
                "pattern": "encryption des",
                "remediation": None,
            },
            {
                "name": "No 3DES-only crypto maps",
                "type": "must_not_contain",
                "pattern": "encryption 3des",
                "remediation": None,
            },
            {
                "name": "AES-256 or AES-GCM in use",
                "type": "regex_match",
                "pattern": r"encryption\s+(aes-256|aes-gcm-256|aes\s+256)",
                "remediation": None,
            },
            {
                "name": "IKEv2 preferred over IKEv1",
                "type": "regex_match",
                "pattern": r"crypto ikev2\s+",
                "remediation": None,
            },
            {
                "name": "SHA-256 or higher hash",
                "type": "regex_match",
                "pattern": r"integrity\s+sha(256|384|512)|hash\s+sha(256|384|512)",
                "remediation": None,
            },
            {
                "name": "DH group 14 or higher",
                "type": "regex_match",
                "pattern": r"group\s+(1[4-9]|2[0-4])",
                "remediation": None,
            },
            {
                "name": "No MD5 authentication",
                "type": "must_not_contain",
                "pattern": "authentication md5",
                "remediation": None,
            },
            {
                "name": "IPsec lifetime reasonable",
                "type": "regex_match",
                "pattern": r"set security-association lifetime seconds\s+\d+",
                "remediation": None,
            },
        ],
    ),
    ...
    (
        ...
        [
            {
                "name": "No default 'public' community",
                "type": "must_not_contain",
                "pattern": "snmp-server community public",
                "remediation": ["no snmp-server community public"],
            },
            {
                "name": "No default 'private' community",
                "type": "must_not_contain",
                "pattern": "snmp-server community private",
                "remediation": ["no snmp-server community private"],
            },
            {
                "name": "SNMPv3 auth-priv group",
                "type": "regex_match",
                "pattern": r"snmp-server group\s+\S+\s+v3\s+priv",
                "remediation": ["snmp-server group PLEXUS-RO v3 priv"],
            },
            {
                "name": "SNMPv3 user configured",
                "type": "regex_match",
                "pattern": r"snmp-server user\s+\S+\s+\S+\s+v3\s+auth\s+sha",
                "remediation": None,
            },
            {
                "name": "SNMP community with ACL restriction",
                "type": "regex_match",
                "pattern": r"snmp-server community\s+\S+\s+(RO|RW)\s+\S+",
                "remediation": None,
            },
            {
                "name": "SNMP trap host configured",
                "type": "regex_match",
                "pattern": r"snmp-server host\s+\S+",
                "remediation": ["snmp-server host 10.0.0.50 version 3 priv PLEXUS-RO"],
            },
            {
                "name": "SNMP contact set",
                "type": "regex_match",
                "pattern": r"snmp-server contact\s+\S+",
                "remediation": ["snmp-server contact NetworkOps"],
            },
            {
                "name": "SNMP location set",
                "type": "regex_match",
                "pattern": r"snmp-server location\s+\S+",
    ...
    (
        ...
        [
            {
                "name": "Hostname configured (not default)",
                "type": "regex_match",
                "pattern": r"hostname\s+(?!Router|Switch)\S+",
                "remediation": None,
            },
            {
            ...
            {
                "name": "DNS server configured",
                "type": "regex_match",
                "pattern": r"ip name-server\s+\d+\.\d+\.\d+\.\d+",
                "remediation": ["ip name-server 10.0.0.50"],
            },
            {
                "name": "Clock timezone configured",
                "type": "regex_match",
                "pattern": r"clock timezone\s+\S+",
                "remediation": ["clock timezone EST -5", "clock summer-time EDT recurring"],
            },
            {
                "name": "Memory free low-watermark",
                "type": "regex_match",
                "pattern": r"memory free low-watermark",
                "remediation": ["memory free low-watermark processor 10000"],
            },
            {
                "name": "Archive logging enabled",
                "type": "regex_match",
                "pattern": r"archive[\s\S]*?log config",
                "remediation": ["archive", "log config", "logging enable", "logging size 200", "hidekeys"],
            },
            {
                "name": "No plaintext passwords in config",
                "type": "must_not_contain",
                "pattern": "password 0 ",
                "remediation": ["service password-encryption"],
            },
            {
                ...
                "type": "regex_match",
                "pattern": r"crypto key generate|crypto pki",
                "remediation": None,
            },
        ],
    ),
