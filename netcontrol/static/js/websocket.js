    ...
    ws.onmessage = (event) => {
        try {
            ...
            if (data.type === 'campaign_complete') {
                ...
                // After replay, skip live events that duplicate replayed history
                if (replayDone && lastReplayEventId && data.event_id && data.event_id <= lastReplayEventId) {
                    return;
                }
                if (onMessage) onMessage(data);
            }
