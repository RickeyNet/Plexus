    ...
    async def _go():
        ...
        for i in range(4):
            await db_module.create_interface_ts_sample(
                h1, 10, "Gi0/1", 1000, 100 * i, 200 * i, in_rate_bps=float(i), out_rate_bps=float(i)
            )
            await db_module.create_interface_ts_sample(
                h1, 11, "Gi0/2", 1000, 300 * i, 400 * i, in_rate_bps=float(10 + i), out_rate_bps=float(i)
            )
            await db_module.create_interface_ts_sample(
                h2, 10, "Gi0/1", 1000, 500 * i, 600 * i, in_rate_bps=float(20 + i), out_rate_bps=float(i)
            )

        res = await db_module.query_interface_ts_multi(
            [(h1, 10), (h1, 11), (h2, 10)], start="2000-01-01 00:00:00", limit_per=2
        )
